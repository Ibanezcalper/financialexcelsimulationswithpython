...
output_folder = r'C:\06_Py\05_TRAD03_DOR06\01_salidas\01_COP'
...
with open(input_file_a, newline='', encoding='utf-8-sig') as csvfile:
    ...
    for row in tqdm(rows, desc="Procesando registros", total=total_rows):  # Añadir tqdm para la barra de progreso
        ...
        # Datos adicionales
        cost_sevicios_cdh = to_number(row['cost_sevicios_cdh'])
        cost_servicios_rest = to_number(row['cost_servicios_rest'])
        cost_servicios_comercial = to_number(row['cost_servicios_comercial'])
        cost_nomina_t = to_number(row['cost_nomina_t'])
        cost_imp_benef_nomina = to_number(row['cost_imp_benef_nomina'])

        ing_renta_hab = to_number(row['ing_renta_hab'])
        ing_renta_comercial = to_number(row['ingresos_comercial'])
        ...
        zona = row['zona'].lower()  # Obtener la zona y convertirla a minúsculas
        ...
        if estado_uno in filtro_estatus:
            ...
            sheet_c = wb_b['PresupuestoCost'] 
            sheet_d = wb_b['PresupuestoIng']
            sheet_e = wb_b['Flujo']
            ...
            # sheet_c
            sheet_c['F28'].value = cost_servicios_comercial / 1000
            sheet_c['F28'].number_format = '0.00' if isinstance(cost_servicios_comercial, float) else '@'

            # sheet_d  
            sheet_d['F8'].value = ing_renta_comercial / 1000
            sheet_d['F8'].number_format = '0.00' if isinstance(ing_renta_comercial, float) else '@'

            # sheet_e
            sheet_e['K1'].value = apreciacionInmobiliaria
            sheet_e['K1'].number_format = '0.00' if isinstance(apreciacionInmobiliaria, float) else '@'
            sheet_e['E3'].value = porcentajeAnho1
            sheet_e['E3'].number_format = '0.00' if isinstance(porcentajeAnho1, float) else '@'              
                
            # Guardar el nuevo archivo
            output_file_name = f'{predio}_APRECIACION.{apreciacionInmobiliaria:.2f}_B.NRM.REHAB.SCOM.xlsx'
